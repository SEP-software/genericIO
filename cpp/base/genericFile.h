...
class generic_reg_file: public param_func{
  public:
    ...
    virtual void put_float(const std::string par, const float val){if(par=="" && val==0);}
    virtual void put_string(const std::string par, const std::string val){if(par=="" && val=="");}
    virtual void put_bool(const std::string par, const bool val){if(par=="" && val==0) ;}
    virtual void put_ints(const std::string par, const  std::vector<int> val){if(par=="" && val[0]==0);}
    virtual void put_floats(const std::string par, const std::vector<float> val) {if(par=="" && val[0]==0);}
    
    virtual void read_float_stream(const long long npts, float *array){ if(npts==0 && array==0);}
    virtual void write_float_stream(const long long npts, const float *array){if(npts==0 && array==0);}
