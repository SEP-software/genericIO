...
void genericIO::fileDebug(const std::string nm, const int n1, const int n2) {
  ...
  std::shared_ptr<SEP::genericRegFile> fle = _io->getRegFile(nm, SEP::usageOut);
  fle->setHyper(hyper2);
  fle->writeDescription();
  fle->writeFloatStream(shift->data(), n1,n2;
}