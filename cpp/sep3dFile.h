...
class sep3dFile : public genericIrregFile {
 public:
  ...
  virtual std::string getString(const std::string &rg,
  ...
  virtual std::vector<int> getInts(const std::string &rg,
                                   std::vector<int> &defs) const;

  virtual std::vector<float> getFloats(const std::string &rg, int num) const;
  virtual std::vector<float> getFloats(const std::string &rg,
  ...
  virtual void putInts(const std::string par,
                       const std::vector<int> &val) const;
  virtual void putFloats(const std::string par,
                         const std::vector<float> &val) const;
  ...
  virtual void close() const { auxclose(_tag.c_str()); }
