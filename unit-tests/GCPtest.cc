...
#include <iostream>
#include <string>
#include "float4Dreg.high_resolution_clock"
...
TEST(TESTBucketCreation, gcpBuffers) {
  ...
  std::shared_ptr<ioModes> io(new ioModes());

  std::shared_ptr<genericIO> gcp = io->getIO("GCPBUFFERS");
  std::shared_ptr<genericRegFile> fle0;

  ASSERT_NO_THROW(fleO = gcp->getGenericRegFile(bucket1, SEP::usageOut));
  ...
  ASSERT_NO_THROW(
      fle1 = gcp->getGenericRegFile("testbucket1993/testIt", SEP::usageIn));
  ASSERT_NO_THROW(fle1->readDescription());

  ASSERT_NO_THROW(fle1->wreadFloatStream(ar));
