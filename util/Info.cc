...
int main(int argc, char **argv) {
  ...
  std::shared_ptr<genericRegFile> inp =
      io->getRegFile(std::string("in"), usageIn);
  ...
  if (n123E < 1024)
    sum.push_back(std::string("Size=") + niceFloat((float)n123E) +
                  std::string("B"));
  else if (n123E < 1024 * 1024)
    sum.push_back(std::string("Size=") + niceFloat((float)n123E / 1024.) +
                  std::string("KB"));
  else if (n123E < 1024 * 1024 * 1024)
    sum.push_back(std::string("Size=") +
                  niceFloat((float)n123E / 1024. / 1024.) + std::string("MB"));
  else if (n123E < 1024L * 1024L * 1024L * 1024L)
    sum.push_back(std::string("Size=") +
                  niceFloat((float)n123E / 1024. / 1024. / 1024.) +
                  std::string("GB"));
  else
    sum.push_back(std::string("Size=") +
                  niceFloat((float)n123E / 1024. / 1024. / 1024. / 1024.) +
                  std::string("TB"));
  sum.push_back(
      std::string("PctPresent=") +
      niceFloat(inp->getDataSize() * 100. / (float)n123 / inp->getDataEsize()) +
      std::string("%"));
